...
class Common
{
    ...
    function backup_tables($host, $user, $pass, $name, $tables = '*')
    {
        ...
        if (!file_exists(storage_path("db-backups"))) {
            @mkdir(storage_path("db-backups"));
        }
    ...
    public function pretty_url($str, $MODEL = null)
    {
        ...
        while (1) {
            $i++;
            if ($MODEL)
                $cnt = $MODEL::where('slug', $turl)->count();
            else
    ...
    public function getActivityPrice($activityId, $checkIn, $checkOut, $guestCount, $force = false)
    {
        $checkIn = setDateForDb($checkIn);
        $checkOut = setDateForDb($checkOut);
        $from = new DateTime($checkIn);
        ...
        $enddate = date('Y-m-d', strtotime($checkOut));
        $date2 = date('Y-m-d', (strtotime('-1 day', strtotime($enddate))));
        $totalDays = date_diff($from, $to)->format('%a');
        $dates = $this->get_days($date1, $date2);
        $activityDates = ActivityDate::where(['activity_id' => $activityId])->whereIn('date', $dates)->get();
        if ($activityDates->where('status', 'Not available')->count() > 0 && !$force) {
            $result['status'] = "Not available";
            return json_encode($result);
        }
        $activity = Activity::with('price')->find($activityId);
        // dd($activity->price);
        $result['activity_default'] = [
            'price' => $activity->price->price,
            'currency_code' => $activity->price->currency_code,
            'symbol' => Currency::getAll()->firstWhere('code', $activity->price->currency_code)->symbol,
            'rate' => Currency::getAll()->firstWhere('code', $activity->price->currency_code)->rate,
            'local_to_propertyRate' => $this->currency_rate($activity->price->currency_code, $this->getCurrentCurrencyCode()),
        ];
        $differentPriceDatePrice = $force ? $activityDates->pluck('price', 'date')->toArray() : $activityDates->where('status', 'Available')->pluck('price', 'date')->toArray();
        $differentPrice = 0;
        $weekendPrice = 0;
        $dayPrice = 0;
        $monthPrice = 0;
        ...
        foreach ($dates as $date) {
            if (in_array($date, array_keys($differentPriceDatePrice))) {
                $differentPrice += $this->convert_currency($activity->price->currency_code, '', $differentPriceDatePrice[$date]  * $guestCount);
            } else {
                $dayPrice += $activity->price->price * $guestCount;
            }
            $countDays++;
        }
        ...
        foreach ($dates as $key => $value) {
            if (in_array($value, array_keys($differentPriceDatePrice))) {
                $allDate[$key]['price'] = moneyFormat($symbol, numberFormat($this->convert_currency($activity->price->currency_code, '', $differentPriceDatePrice[$value]  * $guestCount), $decimals));
                $allDate[$key]['original_price'] = $this->convert_currency($activity->price->currency_code, '', $differentPriceDatePrice[$value]  * $guestCount);
            } else {
                $allDate[$key]['price'] = moneyFormat($symbol, numberFormat($activity->price->price  * $guestCount, $decimals));
                $allDate[$key]['original_price'] = $activity->price->price  * $guestCount;
            }
        ...
        $propertyFees = PropertyFees::pluck('value', 'field');
        $remainingDayPrice = ($differentPrice + $weekendPrice + $dayPrice) - $monthPrice - $weekPrice;

        $result['total_person_price'] = ($remainingDayPrice + $monthPrice + $weekPrice);
        $result['total_day_price'] = $result['total_person_price'];
        $result['discount'] = round(($monthPrice * $activity->price->discount) / 100, $decimals) + round(($weekPrice * $activity->price->weekly_discount) / 100, $decimals);
        $result['property_price'] = round($result['total_day_price'] / $countDays, $decimals);
        $result['total_persons'] = $guestCount;
        $result['total_days'] = $countDays;
        $result['service_fee'] = round(($propertyFees['guest_service_charge'] / 100) * $result['total_day_price'], $decimals);

        $result['total'] = $result['service_fee'] + $result['total_day_price'] - $result['discount'];
        $result['subtotal'] = $result['total'];

        $result['total_day_price_with_symbol'] = moneyFormat($symbol, numberFormat($result['total_day_price'], $decimals));
        $result['total_person_price_with_symbol'] = moneyFormat($symbol, numberFormat($result['total_person_price'], $decimals));
        $result['service_fee_with_symbol'] = moneyFormat($symbol, numberFormat($result['service_fee'], $decimals));
        $result['total_with_symbol'] = moneyFormat($symbol, numberFormat($result['total'], $decimals));
        $result['per_day_price_with_symbol'] = moneyFormat($symbol, numberFormat($result['total_day_price'] / $countDays, $decimals));
        $result['discount_with_symbol'] = moneyFormat($symbol, numberFormat($result['discount'], 2));
        $result['currency'] = $this->getCurrentCurrencyCode();
