...
class UserController extends Controller
{
    ...
    {
        $data['notifications'] = Notifications::where('user_id',auth()->id())->orderBy('created_at','desc')->paginate();

        return view('users.notification', $data);
    }
    public function notificationRead($id)
    {
        Notifications::where(['user_id' => auth()->id(), 'id' => $id])->update([
            'status' => 'read'
        ]);

        $this->helper->one_time_message('success', trans('Notification read successfully'));

        return back();
    }
    public function notificationDelete($id = null)
    {
        Notifications::where(['user_id' => auth()->id(), 'id' => $id])->delete();

        $this->helper->one_time_message('success', trans('Notification delete successfully'));
