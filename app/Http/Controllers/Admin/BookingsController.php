...
use App\DataTables\BookingsActivityDataTable;
...
use App\Models\{Activity, BankDate, Bookings, BookingDetails, PropertyDates, PropertyType, Properties, User, Currency, SpaceType, Payouts, Settings, PayoutSetting, Wallet};
...
class BookingsController extends Controller
{
    ...
    public function indexActivity(BookingsActivityDataTable $dataTable, Request $request)
    {
        $data['from'] = isset(request()->from) ? request()->from : null;
        $data['to']   = isset(request()->to) ? request()->to : null;

        if (isset(request()->activity)) {
            $data['activities'] = Activity::where('activities.id', request()->activity)->select('id', 'name')->get();
        } else {
        ...
        //Calculating total customers, bookings and total amount in distinct currency
        $total_bookings_initial  = $this->getAllActivityBookings();
        $total_bookings_currency = $this->getAllActivityBookings();

        $total_bookings = $this->getAllActivityBookings();
        $data['total_bookings']  = $total_bookings->get()->count();
        ...
        if (isset(request()->reset_btn)) {
            ...
            $data['to']              = null;
            $data['allstatus']       = null;
            $data['allactivities']   = null;
            $data['allcustomers']    = null;
            return $dataTable->render('admin.view-activity', $data);
        }
        ...
        if ($activity) {
            $total_bookings_initial = $total_bookings_initial->where('bookings.activity_id', '=', $activity);
            $total_customers_initial = $total_customers_initial->where('bookings.activity_id', '=', $activity);
            $different_currency_total_initial = $different_currency_total_initial->where('bookings.activity_id', '=', $activity);
        }
        if ($customer) {
            $total_bookings_initial = $total_bookings_initial->where('bookings.user_id', '=', $customer);
            $total_customers_initial = $total_customers_initial->where('bookings.user_id', '=', $customer);
        ...
        isset(request()->activity) ? $data['allactivities'] = request()->activity : $data['allactivities'] = '';
        isset(request()->customer) ? $data['allcustomers'] = request()->customer : $data['allcustomers'] = '';
        isset(request()->status) ? $data['allstatus'] = request()->status : $data['allstatus'] = '';
        return $dataTable->render('admin.bookings.view-activity', $data);
    }
    ...
    public function searchActivity(Request $request)
    {
        ...
        if ($str == null) {
            $myresult = Activity::where('status', 'Listed')->select('id', 'name')->take(5)->select('activities.id', 'activities.name AS text')->get();
        } else {
            $myresult = Activity::where('activities.name', 'LIKE', '%' . $str . '%')->select('activities.id', 'activities.name AS text')->get();
        }
    ...
    public function getAllActivityBookings()
    {
        $allBookings = Bookings::join('activities', function ($join) {
            ...
            ->select(['bookings.id as id', 'u.first_name as host_name', 'users.first_name as guest_name', 'activities.name as property_name', DB::raw('CONCAT(currency.symbol, bookings.total) AS total_amount'), 'bookings.status', 'bookings.created_at as created_at', 'bookings.updated_at as updated_at', 'bookings.start_date', 'bookings.end_date', 'bookings.guest', 'bookings.host_id', 'bookings.user_id', 'bookings.total', 'bookings.currency_code', 'bookings.service_charge']);
        return $allBookings;
    }
