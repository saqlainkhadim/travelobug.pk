...
use App\DataTables\PayoutsCommissionsDataTable;
...
use App\Models\{Payouts, Withdrawal, Settings, Properties, Currency, Wallet, PaymentMethods, PropertyFees};
...
class PayoutsController extends Controller
{
    ...
    public function index(PayoutsDataTable $dataTable)
    {
        ...
        $data['totalCommisionAmount'] = Withdrawal::sum('amount') / 2;
    ...
    public function commissions(PayoutsCommissionsDataTable $dt)
    {
        ...
        $data['different_total_amounts']   = $this->getDistinctCurrencyTotalWithSymbol($different_currency_total);
        $data['totalPayouts']       = Withdrawal::where('status', '=', 'Success')->count();
        $data['totalPayoutsAmount'] = Withdrawal::where('status', 'Success')->sum('amount') ?? 0;
        $comm = (Withdrawal::where('status', 'Success')->sum('amount') ?? 0) * ($serviceCharge && $serviceCharge != 0 ? ($serviceCharge / 100) : 0);
        $data['totalCommisionAmount'] = number_format(round($comm, 2), 2);
        ...
        if (isset(request()->reset_btn)) {
            ...
            $data['alltypes']   = '';
            $data['allproperties']   = '';
            return $dt->render('admin.payouts.commissions', $data);
        }
