...
use App\Http\{Helpers\Common, Controllers\Controller, Controllers\EmailController, Requests};
use App\Models\{Activity, Bank, Bookings, BookingDetails, Messages, Penalty, Payouts, Properties, PayoutPenalties, PropertyDates, PropertyFees, Settings, Currency, Country};
...
class BookingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!$data['result'] || $data['result']->host_id != auth()->id()) {

            abort('404');
        }
        if ($data['result']->category !== 'activity') {
            ...
            $data['price_list'] = json_decode($this->helper->getActivityPrice($data['result']->activity_id, $data['result']->start_date, $data['result']->end_date, $data['result']->guest, true));
        }
    ...
    public function requestPayment(Request $request)
    {
        ...
        return $data['category'] == 'activity' ? view('payment.payment-activity', $data) : view('payment.payment', $data);
    }
    ...
    public function requested(Request $request)
    {
        ...
        if ($data['booking_details']->category !== 'activity') {
            ...
            $data['price_list'] = json_decode($this->helper->getActivityPrice($data['booking_details']->activity_id, $data['booking_details']->start_date, $data['booking_details']->end_date, $data['booking_details']->guest, true));
        }
        $data['symbol'] = $this->helper->getCurrentCurrencySymbol();

        return $data['booking_details']->category !== 'activity' ? view('booking.requested', $data) : view('booking.requested-activity', $data);
    }
    ...
    public function accept(Request $request, EmailController $email)
    {
        ...
        $penalty = Penalty::where('user_id', auth()->id())->where('remaining_penalty', '!=', 0)->get();
        ...
        $requestBookingConfirm = ($companyName . ': ' . 'Your booking request for' . ' ' . ($booking->category !== 'activity' ? $booking->properties->name : $booking->activity->name) . ' ' . 'is Accepted, Please Payment for booking.');

        notificationSave(['user_id' => $booking->user_id, 'message' => $requestBookingConfirm]);
    ...
    public function decline(Request $request, EmailController $email)
    {
        ...
        notificationSave(['user_id' => $booking->user_id, 'message' => $requestBookingDecline]);
    ...
    public function myBookings(Request $request)
    {
        ...
        $data['title']   = "Bookings";
        $bookings = Bookings::with(['users', 'properties'])
            ->where('host_id', auth()->id())
            ->where($params)->orderBy('id', 'desc');
        if (request('category') != 'activity') {
            $bookings->where('category', '!=', 'activity');
        } else {
            $bookings->where('category', '=', 'activity');
        }

        $data['bookings'] = $bookings->paginate(Session::get('row_per_page'));
