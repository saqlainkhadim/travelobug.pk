...
{
    return
        // DB::table(DB::raw("(SELECT * from messages where receiver_id=" . Auth::id() . " and `read`=0 ORDER by id DESC) as msg"))
        Messages::with(['bookings'])
        ->where(['receiver_id' => auth()->id(), 'read' => 0])
...
function propertyCategory($cat = null)
{
    if (!$cat || $cat == 'all') {
        return Common::$propertyCategories;
    }
    return Common::$propertyCategories[$cat] ?? $cat;
}

function optimize_proact_image_topath($file, string $path, $quality = 80, int $height = null, int $width = null, string $img_driver = null): string
{
    $name = $file->getClientOriginalName();
    $ext = $file->extension();
    $file_name = uniqid() . '_' . md5($name) . ".$ext";
    $file_path = rtrim($path, '/') . '/' . $file_name;

    // set image driver; default: gd, available: gd, imagick
    if ($img_driver) {
        if (!in_array($img_driver, ['gd', 'imagick'])) {
            throw new \Exception('Unsupported image driver');
        }
        Image::configure(['driver' => $img_driver]);
    }
    ...
    $_width  = (int) ($width ?? 1280);
    $_height = (int) ($height ?? 720);

    $width = $img->height() > $img->width() ? $_height : $_width;
    $height = $img->height() > $img->width() ? $_width : $_height;
    ...
    // $img->resize(960, 540, fn ($constraint) => $constraint->aspectRatio());
    $img->resize($width, $height, fn (Constraint $constraint) => $constraint->aspectRatio());
    // $img->encode('png', 75);
