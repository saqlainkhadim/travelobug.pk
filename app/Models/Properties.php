...
class Properties extends Model
{
    ...
    protected $appends = [
        'steps_completed', 'space_type_name', 'property_type_name',
        'property_photo', 'host_name', 'book_mark', 'reviews_count', 'overall_rating', 'cover_photo', 'avg_rating'
    ];
    ...
    public static function recommendedHotel()
    {
        $data = parent::where('status', 'listed')
            ->with('users', 'property_price', 'property_address')
    ...
    public static function recommendedGuestHouse()
    {
        $data = parent::where('status', 'listed')
            ->with('users', 'property_price', 'property_address')
            ->where('category', 'guest_house')
            ->where('approved', 1)
            // ->where('recomended', '1')
            ->orderBy('recomended', 'desc')
            ->take(8)
...
//    public function getEmbededMapAttribute()
//    {
//        $base64embededMap = $this->getAttribute('embeded_map');
//        return base64_decode($base64embededMap);
//    }
    ...
    {
        $avg = Reviews::where('property_id', $this->attributes['id'])->where('reviewer', 'guest')->avg('rating');
