...
@section('main')
    <div class="container-fluid bg-white main-panel border-0 p-0 mt-70">
        <div class="row">
            ...
            <div class="hidden-pod filter-h col-md-12" id="listCol">
                <div class="row mt-5">
                                                ...
                                                <form id="front-search-form" method="post" action="{{url('search')}}">
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-12">
                                                            ...
                                                            <div class="input-group mt-4">
                                                                <div class="position-relative">
                                                                    {{-- <input class="form-control p-3 text-14 address_input" value="{{$location}}" placeholder="{{trans('messages.home.where_want_to_go')}}" autocomplete="off" name="location" type="text">
                                                                    <div class="address_html"></div> --}}
                                                                    <input class="form-control p-3 text-14 address_input" value="{{$location}}" id="front-search-address" placeholder="{{trans('messages.home.where_want_to_go')}}" autocomplete="off" name="location" type="text">
                                                                    <div class="address_html"></div>
                                                                </div>
                                                            ...
                                                            <div class="row">
                                                                ...
                                                                <div class="col-md-3">
                                                                    ...
                                                                    <select class="form-control text-16"  id="front-search-guests" name="guests">
                                                                        @for($i=1;$i<=16;$i++)
                                                                        <option value="{{ $i }}" {{ request('guest') == $i ? 'selected' : '' }}> {{ ($i == '16') ? $i.'+ ' : $i }} </option>
                                                                        @endfor
                                                                    </select>
                                                                </div>
                                                            </div>
                                                        ...
                                                        <div class="col-md-12 mt-5 text-center">
                                                            <button class="btn vbtn-outline-success text-16 font-weight-700 pl-5 pr-5 pt-3 pb-3" id="location" type="button">
                                            ...
                                            @if($row_inc <= 4)
                                                <div class="col-md-6">
                                                    <div class="form-check mt-4">
                                                        <input type="checkbox" name="amenities[]" value="{{ $row_amenities->id }}" class="form-check-input mt-2 amenities_array" id="map-search-amenities-{{ $row_amenities->id }}" {{ (in_array($row_amenities->id, $amenities_selected)) ? 'checked' : '' }}>
                                    ...
                                    <div class="row mt-2">
                                        ...
                                        @foreach($property_type as $row_property_type =>$value_property_type)
                                            @if($pro_inc <= 4)
                                                <div class="col-md-6">
                                                    <div class="form-check mt-4">
                                                        <input type="checkbox" name="property_type[]" value="{{ $row_property_type }}" class="form-check-input mt-2" id="map-search-property_type-{{ $row_property_type }}" {{ in_array($row_property_type, $property_type_selected) ? 'checked' : '' }}>
    ...
    <script type="text/javascript">
        ...
        function addMarker(map, features){
            ...
            for (var i = 0, feature; feature = features[i]; i++) {
                ...
                var marker = new markerWithLabel.MarkerWithLabel({
                    ...
                    map: map,
                    icon: feature.icon,
                    labelContent: feature.label, // can also be HTMLElement
                    labelAnchor: new google.maps.Point(-22, 0),
                    labelClass: "price_labels", // the CSS class for the label
                    labelStyle: { opacity: 1.0, fontSize: '14px', fontWeight: 700 },
                    title: feature.title !== undefined? feature.title : undefined,
                    content: feature.content !== undefined? feature.content : undefined,
                });
                // var marker = new HTMLMarker(feature.latitude, feature.longitude);
                markers.push(marker);
        ...
        function getProperties(url){
            ...
            var location    = $('#front-search-address').val();
                    ...
                    success: function (result) {
                        ...
                        for (var key in properties) {
                            if (properties.hasOwnProperty(key)) {
                                ...
                                var p_price = (category != 'activities'? properties[key].property_price : properties[key].price);
                                var p_user = (category != 'activities'? properties[key].users : properties[key].user);
                                var p_address = (category != 'activities'? properties[key].property_address : properties[key].address);

                                var p_route = APP_URL+'/properties/'+properties[key].slug;
                                if (category == 'activities') {
                                    p_route = APP_URL+'/activities/'+properties[key].slug;
                                }
                                ...
                                room_point[key] = {
                                    ...
                                    icon: {},
                                    label: `<strong class="d-inline-block bg-white p-1 border border-dark mb-0" style="font-size:1.25rem">
                                            $${p_price.price}
                                        </strong>`,
                                    content: '<a href="'+p_route+'?checkin='+checkin+'&checkout='+checkout+'&guests='+guest+'" class="media-cover" target="_blank">'
                                    ...
                                    +'<div class="map-property-name">'
                                        +'<div class="col-xs-12 p-1">'
                                            +'<div class="location-title">'
                                                +`<h5 class="float-right">${p_price.default_symbol + p_price.price}/night</h5>`
                                                +'<h5>'+properties[key].name+'</h5>'
                                                +'<span><i class="fa fa-star text-14 secondary-text-color"></i>'+' '+ properties[key].avg_rating
                                                +' '+ '('+properties[key].reviews_count+')</span>'
                                            +'</div>'
                                        +'</div>'
                                    +'</div>'
                                };
                                ...
                                if(properties[key].reviews_count == 1) reviews_count = properties[key].reviews_count;
                                    ...
                                    var moneySymbol = p_price.default_symbol;
                                    var price       = p_price.price;
                                    var symbolWithPrice = moneyFormat(moneySymbol, price);
                                    var color = properties[key].book_mark ? 'var(--scheme-color)' : '';
                                    ...
                                    if (divCol == false) {
                                        room_div += '<div class="col-md-6 col-lg-3 p-2 pl-4 pr-4 mt-4">'
                                                    +'<div class="card h-100">'
                                                        +'<div class="grid">'
                                                            +'<a href="'+p_route+'?checkin='+checkin+'&checkout='+checkout+'&guests='+guest+'" target="_blank">'
                                                        ...
                                                        +'<div class="card-body p-0 pl-1 pr-1">'
                                                            +'<div class="d-flex">'
                                                                +'<div>'
                                                                    +'<div class="pl-2 pr-1">'
                                                                        +'<a href="'+APP_URL+'/users/show/'+properties[key].host_id+'"><img src="'+p_user.profile_src+'" class="img-60x60 rounded-circle" alt="profile-image"></a>'
                                                                    +'</div>'
                                                                +'</div>'

                                                                +'<div class="p-2 text">'
                                                                    +'<a class="text-color text-color-hover" href="'+p_route+'?checkin='+checkin+'&checkout='+checkout+'&guests='+guest+'" target="_blank">'
                                                                        +'<h4 class="text-16 font-weight-700 text">' +properties[key].name+'</h4>'
                                                                    +'</a>'
                                                                    +'<p class="text-13 mt-2 mb-0 text"><i class="fas fa-map-marker-alt"></i> '+ p_address.address_line_1+'</p>'
                                                                +'</div>'
                                                            +'</div>'
        ...
        $('#btnBook, #btnRoom, #btnPrice, .filter-apply, #location').on('click', function(){
            allowRefresh = true;
            deleteMarkers();
            loadPage = '{{url("search/result")}}';
            // getProperties($('#map_view').locationpicker('map').map);
            ...
            $('.dropdown-menu-location').removeClass('show');
        });
        ...
        function getCheckedValueArray(field_name){
            ...
            // console.table({field_name, array_Value});
