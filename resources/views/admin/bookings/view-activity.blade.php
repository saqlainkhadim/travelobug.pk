		...
		@section('main')
			<div class="content-wrapper">
				<section class="content-header">
					<h1>{{ request('status', 'All') }} Bookings<small>Control panel</small></h1>
				...
				<section class="content">
					...
					<div class="row">
						<div class="col-xs-12">
							<div class="box">
								<div class="box-body">
									<form class="form-horizontal" enctype='multipart/form-data' action="{{ url('admin/bookings-activity') }}" method="GET" accept-charset="UTF-8">
										{{ csrf_field() }}
												...
												<div class="col-md-12 col-sm-12 col-xs-12">
													...
													<div class="col-md-3 col-sm-3 col-xs-12">
														...
														<select class="form-control select2" name="activity" id="activity">
															...
															@if(!empty($properties))
																@foreach($properties as $activity)
																	<option value="{{$activity->id}}" "{{$activity->id == $allproperties ? ' selected="selected"' : ''}}">{{$activity->name}}</option>
																@endforeach
															@endif
														</select>
													</div>
	...
	// Select 2 for activity search
	$('.select2').select2({
	// minimumInputLength: 3,
	ajax: {
		url: 'bookings-activity/activity_search',
	...
	ajax: {
		url: 'bookings-activity/customer_search',
		processResults: function (data) {
		$('#customer').val('DSD');
		return {
			results: data
	...
	$(function() {
		...
		window.location = "booking/booking_list_csv?to="+to+"&from="+from+"&activity="+activity+"&status="+status+"&customer="+customer;
		});
		//pdf convert
		...
		var to = $('#endDate').val();
		var from = $('#startDate').val();
		window.location = "booking/booking_list_pdf?to="+to+"&from="+from+"&activity="+activity+"&status="+status+"&customer="+customer;
		});
		//reload Datatable
