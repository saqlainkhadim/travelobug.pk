  ...
  <section class="content-header">
    ...
    <div class="row">
      ...
      <div class="box-body">
          ...
          <div class="row">
            <div class="col-md-4">
              <label class="label-large">{{trans('messages.listing_basic.category')}}</label>
              <select name="property_category" data-saving="basics1" class="form-control">
                @foreach (propertyCategory() as $key => $value)
                <option value="{{ $key }}" {{ ($key == $result->category) ? 'selected' : '' }}>{{ $value }}</option>
                @endforeach
              </select>
            </div>
            <div class="col-md-4">
