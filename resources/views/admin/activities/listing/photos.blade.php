                    ...
                    <div class="box box-info">
                        <div class="box-body">
                            ...
                            <form id="img_form" enctype='multipart/form-data' method="post"
                                  action="{{url('admin/activity/listing/'.$result->id.'/'.$step)}}"
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    ...
                                                    <input type="text" image_id="{{$photo->id}}"
                                                           activity_id="{{$result->id}}" id="serial-{{$photo->id}}"
                                                    ...
                                                    @if($photo->cover_photo == 0)
                                                        ...
                                                        <select class="form-control photoId" id="photoId">
                                                            <option value="Yes"
                                                                    ...
                                                                    activity_id="{{$result->id}}">Yes
                                                            </option>
                                                            <option value="No"
                                                                    <?= ($photo->cover_photo == 0) ? 'selected' : '' ?> image_id="{{$photo->id}}"
                                                                    activity_id="{{$result->id}}">No
                                                            </option>
                                                        </select>
                                                    @endif
                                                </div>
                                ...
                                <div class="col-md-12">
                                    <div class="col-md-10 col-sm-6 col-xs-6 l-pad-none text-left">
                                        <a data-prevent-default=""
                                           href="{{ url('admin/activity/listing/'.$result->id.'/amenities') }}"
                                           class="btn btn-large btn-primary">{{trans('messages.listing_description.back')}}</a>
                                    </div>
                                    <div class="col-md-2 col-sm-6 col-xs-6 text-right">
                                        <a href="{{url('admin/activity/listing/'.$result->id.'/pricing')}}"
                                           class="btn btn-large btn-primary next-section-button">
    ...
    @push('scripts')
        ...
        <script type="text/javascript">
            ...
            $(document).on('focusout', '.photo-highlights', function (e) {
                var dataURL = '{{route("activity.photo-message", $result->id)}}';
            ...
            function location_image_upload(result) {
                if (result) {
                    var dataURL = '{{route("activity.photo-delete", $result->id)}}';
            ...
            $(document).on('change', '.photoId', function (ev) {
                // alert('ok');
                var dataURL = '{{ route("activity.make-default-photo") }}';
                var option_value = $(this).val();
                var photo_id = $('option:selected', this).attr('image_id');
                var activity_id = $('option:selected', this).attr('activity_id');
            ...
            $(document).on('change', '.serial', function (ev) {
                var dataURL = '{{route("activity.set-photo-serial")}}';
                var serial = $(this).val();
