			...
			<div class="main-panel min-height mt-4">
				<div class="row justify-content-center">
					...
					<div class="col-md-9 mt-4 mt-sm-0 pl-4 pr-4">
						<form id="lis_location" method="post" action="{{url('activity/listing/'.$result->id.'/'.$step)}}" accept-charset='UTF-8'>
							{{ csrf_field() }}
							<div class="col-md-12 border mt-4 pb-5 rounded-3 pl-4 pl-sm-0 pr-4 pr-sm-0 ">
									...
									<div class="col-md-12 pl-5 pr-5">
										...
										<select id="basics-select-bed_type" name="country" class="form-control" id='country'>
											@foreach($country as $key => $value)
											  <option value="{{ $key }}" {{ ($key == $result->address->country) ? 'selected' : '' }}>{{ $value }}</option>
											@endforeach
										</select>
								...
								<div class="row mt-4">
									<div class="col-md-12 pl-5 pr-5">
										...
										<input type="text" name="address_line_1" id="address_line_1" value="{{ $result->address->address_line_1  }}" class="form-control" placeholder="House name/number + street/road">
								...
								<div class="row mt-4">
									<div class="col-md-6 mt-4 pl-5 pr-5">
										...
										<input type="text" name="address_line_2" id="address_line_2" value="{{ $result->address->address_line_2  }}" class="form-control" placeholder="Apt., suite, building access code">
									</div>

									<div class="col-md-6 mt-4 pl-5 pr-5">
										<label class="label-large">{{trans('messages.listing_location.city_town_district')}} <span class="text-danger">*</span></label>
										<input type="text" name="city" id="city" value="{{ $result->address->city  }}" class="form-control">
										<span class="text-danger">{{ $errors->first('city') }}</span>
									</div>

									<div class="col-md-6 mt-4 pl-5 pr-5">
										<label class="label-large">{{trans('messages.listing_location.state_province')}} <span class="text-danger">*</span></label>
										<input type="text" name="state" id="state" value="{{ $result->address->state  }}" class="form-control">
										<span class="text-danger">{{ $errors->first('state') }}</span>
									</div>

									<div class="col-md-6 mt-4 pl-5 pr-5">
										<label class="label-large">{{trans('messages.listing_location.zip_postal_code')}}</label>
										<input type="text" name="postal_code" id="postal_code" value="{{ $result->address->postal_code }}" class="form-control">
							...
							<div class="col-md-12 p-0 mt-4 mb-5">
								<div class="row justify-content-between">
									<div class="mt-4">
										<a href="{{ url('activity/listing/'.$result->id.'/description') }}" class="btn btn-outline-danger secondary-text-color-hover text-16 font-weight-700 pl-5 pr-5 pt-3 pb-3">
...
@push('scripts')
	...
	{{-- <script type="text/javascript">
		...
		$('#map_view').locationpicker({
			location: {
				latitude: {{$result->address->latitude != ''? $result->address->latitude:0 }},
				longitude: {{$result->address->longitude != ''? $result->address->longitude:0 }}
			},
			radius: 0,
			addressFormat: "",
			inputBinding: {
